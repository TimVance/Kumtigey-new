								...
								<ul class="dropdown" >
										<?foreach($arChildItem["CHILD"] as $arChildItem1){?>
											<li class="menu_item <?if($arChildItem1["SELECTED"]){?> current <?}?>">
												...
												<?}?>
												<a class="section" href="<?=$arChildItem1["SECTION_PAGE_URL"];?>"><span><?=$arChildItem1["NAME"];?></span> <?=$arChildItem1["COUNT_GOODS"]?"<span class='count'>(".$arChildItem1["COUNT_GOODS"].")</span>":""; ?></a>
												<?if($arChildItem1["CHILD"]){?>
													<ul class="dropdown">
														<?foreach($arChildItem1["CHILD"] as $arChildItem2){?>
															<li class="menu_item <?if($arChildItem2["SELECTED"]){?> current <?}?>">
																<a class="section1" href="<?=$arChildItem2["SECTION_PAGE_URL"];?>"><span><?=$arChildItem2["NAME"];?></span> <?=$arChildItem2["COUNT_GOODS"]?"<span class='count'>(".$arChildItem2["COUNT_GOODS"].")</span>":""; ?></a>
															</li>
														<?}?>
													</ul>
												<?}?>
